package com.example.asus.androideatit.Database;
...
public class Database extends SQLiteAssetHelper {
    ...
    public void addToCart(Order order){
        ...
        //Please be careful when create insert , '%s'

        SQLiteDatabase db = getReadableDatabase();
        String query = String.format(
                "INSERT OR REPLACE INTO OrderDetail(UserPhone,ProductId,ProductName,Quantity,Price,Discount,Image) VALUES ('%s' , '%s' , '%s' , '%s' , '%s' , '%s' , '%s');",
                order.getUserPhone(),
                order.getProductId(),
    ...
    public void updateCart(Order order) {
        ...
        //Because we are used %d  , so maybe see error : Id = 1 , where 1 in arabic and this mistake , must be 1 in USA
        //So we add new parameter (Local.US , ...)
        String query = String.format(Locale.US,"UPDATE OrderDetail SET Quantity = '%s' WHERE UserPhone = '%s' AND ProductId = '%s'",
                order.getQuantity(),
                order.getUserPhone(),
                order.getProductId());
        db.execSQL(query);
    ...
    public void increaseCart(String userPhone , String foodId) {
        ...
        //Because we are used %d  , so maybe see error : Id = 1 , where 1 in arabic and this mistake , must be 1 in USA
        //So we add new parameter (Local.US , ...)
        String query = String.format(Locale.US,"UPDATE OrderDetail SET Quantity = Quantity+1 WHERE UserPhone = '%s' AND ProductId = '%s'",
                userPhone,
                foodId
                );
        db.execSQL(query);
